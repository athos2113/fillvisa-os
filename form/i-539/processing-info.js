const formKey = "i539-3";
const validPrefix = "i539Valid-";
const validKey = `${validPrefix}3`;
...
function toggleConditionalSections() {
   const showDetails = needsPetitionDetails();
   const section = document.getElementById("petitionDetailsSection");
   section.classList.toggle("d-none", !showDetails);
   section.querySelectorAll("input").forEach(element => {
      element.disabled = !showDetails;
      if (!showDetails) clearField(element);
   });
   if (!showDetails) {
      const feedback = document.getElementById("form_type-feedback");
