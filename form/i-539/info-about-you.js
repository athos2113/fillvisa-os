const formKey = "i539-1";
const validPrefix = "i539Valid-";
const validKey = `${validPrefix}1`;
...
function toggleConditionalSections() {
   const physicalRequired = needsPhysicalAddress();
   const physicalSection = document.getElementById("physicalAddressSection");
   physicalSection.classList.toggle("d-none", !physicalRequired);
   physicalSection.querySelectorAll("input, select, button").forEach(element => {
      element.disabled = !physicalRequired;
      if (!physicalRequired) clearField(element);
   });

   ["mailing", "physical"].forEach(prefix => {
      const unitIds = prefix === "mailing" ? mailingUnitIds : physicalUnitIds;
      const number = document.getElementById(`${prefix}_number`);
      number.disabled = !unitIds.some(isChecked) || (prefix === "physical" && !physicalRequired);
      if (number.disabled) clearField(number);
   });

   const expiry = document.getElementById("status_expiry_date");
   expiry.disabled = isChecked("granted_duration_status");
   if (expiry.disabled) clearField(expiry);
...
function validateFieldById(id) {
   // The Yes/No group has one validation rule and one shared feedback message.
   if (samePhysicalIds.includes(id)) id = "same_physical_yes";
...
applicantForm.querySelectorAll("[data-clear-unit]").forEach(button => {
   button.addEventListener("click", () => {
      const ids = button.dataset.clearUnit === "mailing" ? mailingUnitIds : physicalUnitIds;
      ids.forEach(id => clearField(document.getElementById(id)));
      toggleConditionalSections();
      saveFormToLocalStorage();
   });
});
