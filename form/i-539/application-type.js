const formKey = "i539-2";
const validPrefix = "i539Valid-";
const validKey = `${validPrefix}2`;
...
function toggleConditionalSections() {
   const showStatus = needsStatusChange();
   const statusSection = document.getElementById("statusChangeSection");
   statusSection.classList.toggle("d-none", !showStatus);
   statusSection.querySelectorAll("input").forEach(element => {
      element.disabled = !showStatus;
      if (!showStatus) clearField(element);
   });

   const total = document.getElementById("total_people");
   const wasOnly = total.readOnly;
   const only = isChecked("applying_number_people_only");
   const family = isChecked("applying_number_people_family");
   total.disabled = !hasPeopleSelection();
   total.readOnly = only;
   total.min = family ? "2" : "1";
   if (only) total.value = "1";
   else if (!family || wasOnly) clearField(total);
